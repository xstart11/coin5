...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a QBO address (e.g. %1)").arg("QXDtxPL319yGKpQ5dKyGA8KQdG4NLPotgo"));
#endif
...
{
    // return if URI is not valid or is no QBO: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::QBO, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert qbo:// to qbo:
    //
    //    Cannot handle this later, because qbo:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::QBO, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open qbo.conf with the associated application */
...
// Links
void hyperlinks_slot1(){QString link1 = "https://qabio.io/"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot2(){QString link1 = "https://masternode.fans/"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot3(){QString link1 = "https://bitcointalk.org/index.php?topic=4810286.0"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot4(){QString link1 = "https://twitter.com/QabioOfficial"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot5(){QString link1 = "https://discord.gg/wBYPxej"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot6(){QString link1 = "https://t.me/Qabio"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot7(){QString link1 = "https://www.facebook.com/QabioProject"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot8(){QString link1 = "https://github.com/qabio/qabio"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot9(){QString link1 = "https://explorer.qabio.io/"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot10(){QString link1 = "https://qabio.io/wp_qabio.pdf"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "QBO.lnk";
}
...
{
    return GetAutostartDir() / "qbo.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a qbo.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the qbo app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add qbo app to startup item list
